 ...
 *
 * AI-organized from the author's notes. On the prevalence figures in
 * `keyFacts`, be precise about what is and isn't established:
 *
 *   - Figures that came from the notes were checked against a source by the
 *     author WHEN THE NOTE WAS WRITTEN. They are not unsourced guesses.
 *   - They have not been re-checked since. Prevalence data shifts, so read
 *     them as correct-as-of-noting, not as current.
 *   - This file was never compared against the notes item by item, so it is
 *     not established that every figure here originated in a note rather
 *     than being supplied by the model.
 *   - The inline WHO / CDC / Epilepsy Foundation attributions are mixed
 *     provenance: some were recorded in the notes, some may have been added
 *     by the model, and which is which can no longer be reconstructed per
 *     item.
 *
 * Two earlier versions of this comment were both wrong in opposite
 * directions — one said the stats had been cross-checked against external
 * authorities, the other said every figure was unverified. Neither was
 * true.
 */
export const DISABILITIES_PROVENANCE = {
  category: 'ai-from-notes',
  label: 'AI-derived from author\'s notes (statistics checked when noted, not since)',
  citations: [
    'Author\'s Bear notes tagged #a11y/disabilities',
    'Prevalence figures carry inline WHO / CDC / Epilepsy Foundation attributions of mixed provenance — some recorded in the notes, some possibly added by the model',
  ],
  generatedBy: 'Claude Sonnet 4.6',
  generatedAt: '2025-04',
  humanReview: 'Descriptions spot-checked against the author\'s notes. Prevalence figures drawn from those notes were checked against a source by the author when the note was written, but have not been re-checked since, and this file was not compared against the notes item by item.',
  confidence: 'medium',
  limitations: [
    'Prevalence figures were checked when the underlying note was written, not since. Prevalence data shifts year to year, so treat them as correct-as-of-noting rather than current.',
    'This file was never compared against the notes item by item, so it is not established that every figure here came from a verified note rather than being supplied by the model.',
    'The inline source attributions are mixed: some came from the notes, some may have been added by the model, and which is which cannot be reconstructed per item. Verify any figure at the body cited before using it in an exam answer or professional work.',
