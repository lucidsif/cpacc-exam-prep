...
2. **No compilation or bundling step.** What you commit is what runs in the browser and on the local server, unmodified — no transpiler, no bundler. (`npm run build`/`npm run deploy` do run `scripts/build-dist.sh`, which *stages* a curated subset of the source tree into `dist/` for the Cloudflare deploy — copying files, not transforming them. See "Server / Functions" below and `DEPLOY.md`.)
...
graph TD
  ...
  H[index.html<br/>page shell: skip link, home button,<br/>&lt;main&gt;, #route-status + #route-alert live regions]
...
- `index.html` is the page shell — skip link, home button, a `<main>` element, and two persistent, static live regions that sit outside `<main>` so `innerHTML` rewrites can't destroy them: `#route-status` (`role="status"`, always polite) and `#route-alert` (`role="alert"`, always assertive) — see "The render loop" below for why there are two rather than one region whose politeness gets mutated per call.
...
`announce(msg, { assertive })`, also in `main.js`, writes to one of two persistent, static live regions in `index.html`, both siblings of `<main>` so `app.innerHTML` rewrites can't destroy them: `#route-status` (`role="status"`, always polite) or `#route-alert` (`role="alert"`, always assertive) — `announce()` just picks which one to write into, rather than mutating a single region's `aria-live` per call. Two static regions exist instead of one mutable one because nothing obliges assistive tech to re-read a region's politeness after it's already registered with the accessibility tree, and `role="status"` combined with `aria-live="assertive"` would be self-contradictory the instant it happened. Between them they carry the submit verdict, chat replies/errors, and the "that page isn't available" popstate fallback message — `sendChat`/`sendHomeChat` pass `assertive: true` for a failed reply, since the user is actively waiting on it. In each region, a pending message is cleared, then re-set on a 100ms `setTimeout` (not `requestAnimationFrame`) so that announcing the same string twice in a row still fires: several assistive technologies diff a live region's accessible text before deciding whether to speak it, so writing the identical string twice back-to-back can go unannounced the second time even though the underlying text node was genuinely replaced both times; clearing first, then setting again on the next tick, guarantees the text actually differs immediately beforehand. A `requestAnimationFrame` clear-then-set would risk landing both in the same accessibility-tree update, which could coalesce them into what looks like one no-op mutation. The clear step itself is never announced on its own — default `aria-relevant` is `"additions text"`, so a live region only speaks text that was *added*, not text removed; emptying `textContent` doesn't qualify, only the set that follows does. (This corrects an earlier, less precise version of this explanation, which said "`aria-live` only speaks on a text change" — assigning `textContent` always recreates the text node; the actual mechanism is the `aria-relevant` default, not some notion of "no change happened.") Each region also auto-clears itself 3 seconds (`CLEAR_AFTER_MS`) after being set, so a full reply doesn't sit there permanently — both regions are outside every landmark, and a virtual-cursor user reading to the end of the document used to hit an unlabelled, unexplained duplicate of the last announcement before this was added. See the comment above `announce()`/`makeRegionAnnouncer()` in `src/main.js` for the full reasoning.
...
graph LR
  ...
  U --> Ud[llm.test.js]
  U --> Ue[router.test.js]
  V --> Va[index.html shell<br/>has skip link, main, route-status + route-alert]
...
| File | Covers |
|---|---|
| `smoke.spec.js` | Baseline: the app boots and the home route renders |
...
- **No bundler, no linter, no formatter in `package.json`?** Intentional — the goal is readability over tooling. (There are 9 scripts as of writing — `start`, `test`, `test:e2e`, `test:e2e:headed`, `test:e2e:ui`, `build`, `deploy`, `deploy:preview`, `dev:cf` — all thin wrappers around the test runner, Playwright, `scripts/build-dist.sh`, and `wrangler`, not a build/lint/format toolchain.)
