...
- Both chat transcript containers — `#home-log` (`src/views/home.js`) and `#log-<id>` (`renderChatFragment` in `src/views/chat.js`) — carry `role="log"` **and** an explicit `aria-live="off"` together. This pairing looks self-contradictory (`role="log"` implies `aria-live="polite"`) but is deliberate: both transcripts are rebuilt wholesale via `innerHTML` on every render, and replies are announced through `#route-status` instead, so `aria-live="off"` is what makes `#route-status` the single deterministic announcement path rather than leaving double-announcement to chance. The test message spells this out so a future reader who deletes the attribute to "fix" the apparent contradiction sees the reasoning in the failure output, not just a red test.
