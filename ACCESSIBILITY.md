...
- Changing flashcards with Prev, Next, or Shuffle used to be silent: only flipping a card announced anything. All three now announce the new card's content.
- The AI provenance badge's visible label and confidence level used to be swallowed into the accessible name by an overriding `aria-label`, the same class of bug as the flashcard fix above. Both are now real visible text with the disambiguating label appended via `.sr-only` instead of replacing the content.
- Unsent text typed into a chat box used to be silently destroyed if any other in-place update redrew the page (e.g. opening a second question's chat while the first still had a draft in progress). Chat drafts are now backed in `src/state.js` alongside chat-open and flashcard-flip state, and survive a re-render.
- The fixed-height box a chat transcript scrolls within (`#home-log` in `src/views/home.js`, `#log-${q.id}` in `src/views/chat.js`) is now keyboard-focusable (`tabindex="0"`), so a keyboard user can scroll back through a conversation that has grown past it without a pointer.
- Landmark regions were added app-wide, and each route sets its own `document.title` via `titleFor()` in `src/main.js` (see "Test coverage" below for how this is pinned).
- The "About AI in this app" dialog's close fallback used to focus `<main>` with no visible ring, when its original trigger button was no longer on the page (e.g. removed by an intervening re-render). It now goes through `focusWithVisibleRing()` and lands on the route's own heading, with a ring, like every other script-driven focus move in the app.
- The current-question indicator in the jump grid was drawn with `box-shadow`, which Windows High Contrast / `forced-colors` mode strips. It now has an explicit outline in that mode. This was the only indicator in the app built on `box-shadow` alone; every other colour-only state cue already used `border-color`, which `forced-colors` mode does not strip.
- Browsers without `:focus-visible` support used to lose the focus ring entirely: only the `@supports` narrowing rule existed, with no unconditional base rule underneath it, so a non-supporting engine matched neither rule. A base `:focus` rule now paints the ring unconditionally, and the `@supports` block narrows that down to keyboard-ish focus only in engines that do support `:focus-visible`.
...
- **Cross-device missed-question sync requires the local Node deploy.** The Cloudflare Pages deploy falls back to per-browser `localStorage`. This is a UX limitation, not an a11y limitation.
