...
- **`restoreFocus()` (`src/main.js`) called `.focus()` and declared success unconditionally.** `.focus()` is a spec-legal no-op — not an error — on an element whose `tabindex="-1"` was applied imperatively (as `moveFocusToRoute()` and the results/disabilities/legal jump handlers all do) and had just been erased by the same re-render's `innerHTML` write. This dropped focus to `<body>` on every route it hit, silently, because nothing checked `document.activeElement` afterward — it does now. Both existing positional-fallback regression tests happened to use natively-focusable elements (a radio, a provenance `<summary>`), so `.focus()` succeeded there and the bug stayed invisible until this pass checked the actual outcome instead of trusting the call.
- **The sticky reference-page anchor bar could still obscure the item a user had just jumped to (WCAG 2.4.11, Focus Not Obscured), and this was never confined to the one jurisdiction the previous statement singled out.** The fixed 60px `scroll-margin-top` assumed the anchor bar above it always stayed short; measured directly, it wraps to as much as 389px tall (1280px wide) and 1015px tall (320px wide) on the jurisdiction with the most entries — and covered the focused item in 14 of 16 measured route/viewport combinations, on the disabilities reference too. `.dis-anchor-bar` is now capped at `30vh` and `.dis-item`'s `scroll-margin-top` is set to match that cap, so the two values stay coupled by construction instead of by whichever anchor list happened to fit when someone last measured it. Bundled in the same commit: the anchor bar's `role="group"`, `aria-label`, and `tabindex="0"` are now consolidated onto the one scrollable element instead of split across nested ones — a keyboard-operability robustness improvement, not the closure of a live axe finding (the bar's own focusable anchor buttons already satisfied `scrollable-region-focusable`).
- **The published focus-ring fix from the audit below covered route navigation only.** Four other script-driven focus moves — the post-answer result region (`#verdict`), the results jump grid, and both reference pages' scroll-to-item targets — painted no outline at all, in any of Chromium, Firefox, or WebKit, before this pass. All four now go through the same `focusWithVisibleRing()` helper (`src/dom.js`) that mirrors `moveFocusToRoute()`; confirmed solid 3px `rgb(255, 212, 121)` in all three engines afterward, with `:focus-visible` measuring `false` throughout.
- **That measurement also corrected how the ring's mechanism was described, in this file, `ACCESSIBILITY.md`, and the public statement.** The previous wording said the ring rendered "once a real key has been pressed earlier in the session," framing `:focus-visible` keyboard-modality matching as the cause. For the four newly-fixed targets, `document.activeElement.matches(':focus-visible')` is `false` in every engine and every session state tested — it was never the reason those rings paint. For the two pre-existing route-navigation targets, `:focus-visible` does independently measure `true` in the specific scenario `e2e/focus-contract.spec.js` exercises (a real keypress establishing modality before a keyboard-driven activation) — but the same suite proves the identical ring paints in a cold, mouse-only session where it measures `false`, which is the actual point: `.route-focus:focus`, not the browser's own heuristic, is what every one of these six focus destinations depends on. All three documents are corrected to say so instead of implying `:focus-visible` is the mechanism.
- **The provenance "About AI in this app" trigger had two instances; only one was fixed by the audit below.** The home-page instance got an underline there; the identical trigger inside the AI-provenance banner (`provenance.js`) did not, because `button.linkish`'s `color: inherit; font: inherit` (specificity 0,1,1) beat the intended rule (0,1,0) and silently won. Measured, it rendered in the banner's own text colour at regular weight — visually identical to its surrounding sentence, not merely low-contrast. Re-selectored to `button.pv-info-link` to win on source order. Axe could not have caught this: `link-in-text-block` only fires on `<a>`, and this trigger is a `<button>`.
- **`<details class="provenance">` collapsed on every in-place re-render** — the one piece of user-set UI state that had not yet been moved into `src/state.js` (chat drafts, chat-open, and flashcard-flip state already had been). A chat reply landing while a user was reading a citations list under a screen reader closed that list out from under them. Now backed by `state.expandedProvenance`.
- **Two pieces of help text could go stale and actively mislead.** `#submit-help` ("Select a choice to enable Submit answer") could resurface after a choice was already selected and Submit was already enabled, because it was keyed only on `revealed` while the code that removed it ran once, in a handler that deliberately does not re-render. `#kbd-hint` kept telling users to press a Submit button that revealing the answer had already removed. Both are now computed from live state on every render instead of being one-shot DOM patches.
- **The AI-info dialog's close fallback focused bare `#app` with no visible ring; it now lands on the route's own `<h1>`** via `focusWithVisibleRing()`, matching every other script-driven focus move in the app.
- **The `.toggle` chat-discussion buttons were re-confirmed not to be a WCAG 2.5.8 failure**, and `styles/app.css`'s header comment — which claimed "cells, anchors, and pills" all meet target size, reading as though 2.5.8 had been checked everywhere — now says precisely what was checked and names `.pill` as a non-control `<span>` with no target size to meet.
- jsdom: 151 → 157. End-to-end: 126 → 130 (still five projects, still 74 axe scans — the new tests cover the fixes above, not new scan surface).

Known limitations named in the audit entry below that are resolved by this pass: the `<details open>`-on-re-render loss, and the 60px sticky-anchor-bar `scroll-margin-top`. Still open, unchanged: no `aria-busy`/pending state during chat round trips, and no screen reader testing of the current version — see the public statement's "What has not been verified."
...
- jsdom 131 → 151; end-to-end 60 → 126 across five projects, adding a 320px viewport project and a chat-enabled project. axe now also runs its `best-practice` ruleset — 74 scans total
  ...
  **Amended 2026-08-01 (later the same day):** counts grew again within the same day — see "Fixed — second remediation round" below. jsdom is now 157, end-to-end is now 130 (still five projects, still 74 axe scans).
...
**Amended 2026-08-01 (later the same day):** two of the four items in that sentence were fixed within hours by the follow-up entry below, "Fixed — second remediation round" — the `<details open>` state and the fixed 60px `scroll-margin-top` are no longer accurate as open limitations, and the anchor-bar measurement turned out to be far broader than one jurisdiction. The `aria-busy` gap and the screen-reader-testing gap are unchanged.
...
- These fixes are verified by jsdom unit tests only. The original defects were confirmed by hand in headless Chromium; the fixes have not yet been re-confirmed there. Real browser behaviour generally, actual screen reader announcement (NVDA/JAWS/VoiceOver), focus-ring visibility, and `:focus-visible` matching on programmatic focus in Safari and Firefox remain unverified. See `ACCESSIBILITY.md`
  ...
  **Amended 2026-08-01:** real-browser and cross-engine verification of the focus-ring behaviour has since happened (see "Added — Playwright end-to-end suite" below and its own amendment) — screen reader announcement is the one item in this bullet still genuinely unverified.
